...
class DlibDetector(FaceDetector):
	def __init__(self, device, path_to_detector=None, verbose=False, config = None):
		super().__init__(device, verbose)

		base_path = os.path.join(appdata_dir('face_alignment'), "data")

		if path_to_detector is not None:
			count = 68
			if config is not None and 'dlib_count' in config:
				count = int(config['dlib_count'])
			shape_predictor_file = 'shape_predictor_{}_face_landmarks.dat'.format(count)
			predictor_path = os.path.join(os.path.dirname(path_to_detector), shape_predictor_file)
		...
		if 'cuda' in device:
			...
			if predictor_path is not None and os.path.exists(predictor_path):
				self.predictor = dlib.shape_predictor(predictor_path)
			else:
				print('WARN: predictor_path is null or not exists')
	...
	def predict_from_image(self, tensor_or_path):
		image = self.tensor_or_path_to_ndarray(tensor_or_path, rgb=False)

		dets = self.face_detector(cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))
		...
		for k, d in enumerate(dets):
			...
			faces = [ [shape.part(i).x, shape.part(i).y] for i in range(shape.num_parts)]
			objects.append({
				'location': {
					'x': d.left(),
					'y': d.top(),
